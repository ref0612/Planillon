...
class EditServiceHandler {
    ...
    handleSaveChanges(event) {
        ...
        try {
            ...
            const routeChanged = this.requiresRouteReview(formData);

            // Si ambos cambian, advertir y forzar flujo
            if (busChanged && routeChanged) {
                this.showWarning('Has cambiado tanto la ruta como el bus. Primero revisa las etapas de la nueva ruta y luego reasigna los asientos.');
                this.showRouteReviewStep(formData, () => {
                    this.showSeatReassignmentStep(formData);
                });
                return;
            }

            // Si solo cambia la ruta
            if (routeChanged) {
                this.showRouteReviewStep(formData);
                return;
            }
    ...
    requiresRouteReview(formData) {
        // Simulación: comparar ruta original y nueva
        const originalRoute = this.currentServiceData?.ruta;
        const newRoute = formData.ruta;
    ...
    showRouteReviewStep(formData, onComplete) {
        // Agregar pestaña de revisión de ruta si no existe
        const modal = document.getElementById('editServiceModal');
        if (!modal) {
            console.error('Modal no encontrado');
            return;
        ...
        // Agregar pestaña de etapas de ruta si no existe
        if (!tabsContainer.querySelector('#route-review-tab')) {
            const routeTab = document.createElement('li');
            routeTab.className = 'nav-item';
            routeTab.innerHTML = `
                <button class="nav-link" id="route-review-tab" data-bs-toggle="tab" data-bs-target="#route-review" type="button" role="tab">
            ...
            const routeTabPane = document.createElement('div');
            routeTabPane.className = 'tab-pane fade';
            routeTabPane.id = 'route-review';
            routeTabPane.setAttribute('role', 'tabpanel');
            routeTabPane.innerHTML = this.getRouteReviewHTML(formData);
            tabContentContainer.appendChild(routeTabPane);
        }

        // Cambiar a la pestaña de etapas de ruta
        ...
        tabsContainer.querySelector('#route-review-tab').classList.add('active');
        tabContentContainer.querySelector('#route-review').classList.add('show', 'active');

        // Botón para finalizar revisión de etapas
        const modalFooter = modal.querySelector('.modal-footer');
        if (modalFooter && !modalFooter.querySelector('#finalizarEtapasBtn')) {
            const finalizarBtn = document.createElement('button');
            finalizarBtn.type = 'button';
            finalizarBtn.className = 'btn btn-primary';
            finalizarBtn.id = 'finalizarEtapasBtn';
            finalizarBtn.textContent = 'Finalizar revisión de etapas';
            finalizarBtn.onclick = () => {
                // Si hay callback (caso bus y ruta), continuar con reasignación de asientos
                if (typeof onComplete === 'function') {
                    ...
                    tabsContainer.querySelectorAll('.nav-link').forEach(btn => btn.classList.remove('active'));
                    tabContentContainer.querySelectorAll('.tab-pane').forEach(pane => pane.classList.remove('show', 'active'));
                    tabsContainer.querySelector('#service-details-tab').classList.add('active');
                    tabContentContainer.querySelector('#service-details').classList.add('show', 'active');
                }
                finalizarBtn.remove();
            };
            modalFooter.appendChild(finalizarBtn);
        }
    }

    getRouteReviewHTML(formData) {
        // --- NUEVO FLUJO MIXTO ---
        // Usar variable local para etapas y exponer funciones de agregar/eliminar
        if (!window._dummyEtapas) {
            window._dummyEtapas = [
                { origen: 'Mejillones', destino: 'Terminal de Antofagasta', distancia: 0, duracion: '01:00', espera: '00:00', llegada: '-', embarque: '00:00', subida: true, bajada: false, integracion: true },
                { origen: 'Terminal de Antofagasta', destino: 'Baquedano', distancia: 0, duracion: '01:15', espera: '00:50', llegada: '01:00', embarque: '01:50', subida: true, bajada: false, integracion: true },
                { origen: 'Baquedano', destino: 'Pozo Almonte', distancia: 0, duracion: '03:05', espera: '00:00', llegada: '03:05', embarque: '03:05', subida: true, bajada: false, integracion: true },
                { origen: 'Pozo Almonte', destino: 'Terminal Arica', distancia: 0, duracion: '03:30', espera: '00:00', llegada: '06:50', embarque: '06:50', subida: true, bajada: false, integracion: true },
                { origen: 'Terminal Arica', destino: '', distancia: 0, duracion: '03:30', espera: '00:00', llegada: '10:20', embarque: '-', subida: true, bajada: false, integracion: true }
            ];
        }
        // Renderizar etapas agrupadas por ciudad
        const renderEtapas = () => {
            const container = document.getElementById('route-review');
            if (!container) return;
            container.innerHTML = this.getRouteReviewTableHTML(window._dummyEtapas);
            // Asignar eventos a botones de agregar
            container.querySelectorAll('.btn-add-etapa').forEach(btn => {
                btn.onclick = () => {
                    const ciudad = btn.getAttribute('data-ciudad');
                    const nombreEtapa = prompt('Ingrese el nombre del paradero/etapa a agregar:');
                    if (nombreEtapa && nombreEtapa.trim()) {
                        // Agregar la etapa y asociarla a la ciudad correspondiente
                        window._dummyEtapas.push({
                            origen: nombreEtapa.trim(),
                            destino: '',
                            distancia: 0,
                            duracion: '00:00',
                            espera: '00:00',
                            ...
                            bajada: false,
                            integracion: false,
                            ciudad: ciudad // Asociar explícitamente la ciudad
                        });
                        renderEtapas();
                    }
                };
            ...
            container.querySelectorAll('.btn-del-etapa').forEach(btn => {
                btn.onclick = () => {
                    const ciudad = btn.getAttribute('data-ciudad');
                    const idx = parseInt(btn.getAttribute('data-idx'), 10);
                    // Buscar el idx global de la etapa en window._dummyEtapas
                    let globalIdx = -1, count = -1;
                    for (let i = 0; i < window._dummyEtapas.length; i++) {
                        if (window._dummyEtapas[i].origen === ciudad) {
                            count++;
                            if (count === idx) { globalIdx = i; break; }
                        }
                    }
                    if (globalIdx !== -1) {
                        window._dummyEtapas.splice(globalIdx, 1);
                        renderEtapas();
                    }
                };
            });
        };
        setTimeout(renderEtapas, 100);
    }

    getRouteReviewTableHTML(etapas) {
        // Agrupar etapas por ciudad (dummy)
        const ciudades = [
            { nombre: 'Mejillones', paraderos: ['Mejillones'] },
            { nombre: 'Antofagasta', paraderos: ['Terminal de Antofagasta'] },
            { nombre: 'Baquedano', paraderos: ['Baquedano'] },
            { nombre: 'Pozo Almonte', paraderos: ['Pozo Almonte'] },
            { nombre: 'Arica', paraderos: ['Terminal Arica'] }
        ];
        // Mapear etapas a ciudades usando la propiedad ciudad si existe, si no, por paradero
        const etapasPorCiudad = ciudades.map(ciudad => ({
            ...ciudad,
            etapas: etapas.filter(e => (e.ciudad ? e.ciudad === ciudad.nombre : ciudad.paraderos.includes(e.origen)))
        }));
        // Calcular tiempo total de viaje (sumar duración de todas las etapas)
        let totalMin = 0;
        etapas.forEach(e => {
            const [h, m] = e.duracion.split(':').map(Number);
            totalMin += (h * 60 + m);
        });
        const totalHoras = Math.floor(totalMin / 60);
        const totalMins = totalMin % 60;
        const totalStr = `${totalHoras.toString().padStart(2, '0')}:${totalMins.toString().padStart(2, '0')}`;
        // UI mejorado con Bootstrap y estilos modernos
        return `
        <div class="container-fluid px-0" style="font-size:0.70rem;">
            <div class="row mb-2">
                <div class="col-12 d-flex align-items-center justify-content-between">
                    <h6 class="fw-normal mb-0" style="font-size:0.70rem;letter-spacing:0.2px;">Etapas de la Ruta</h6>
                    <span class="badge bg-primary shadow-sm" style="font-size:0.70rem;padding:5px 8px;">Tiempo total: <b>${totalStr} hrs</b></span>
                </div>
            </div>
            <div class="etapas-list">
                ${etapasPorCiudad.map((ciudad, cidx) => `
                <div class="card shadow-sm mb-2 border-0">
                    <div class="card-header bg-light d-flex justify-content-between align-items-center py-1 px-2" style="border-bottom:1px solid #e3e3e3;min-height:28px;font-size:0.90rem;">
                        <div class="fw-normal" style="font-size:0.90rem;"><i class="fas fa-city me-2 text-secondary"></i>${ciudad.nombre} <small class="text-muted">(${ciudad.etapas.length} Etapa${ciudad.etapas.length!==1?'s':''})</small></div>
                        <button type="button" class="btn btn-success btn-xs btn-add-etapa px-2 py-1" data-ciudad="${ciudad.nombre}" style="font-size:0.70rem;"><i class="fa fa-plus"></i> Añadir</button>
                    </div>
                    <div class="card-body p-2 pb-1" style="font-size:0.70rem;">
                        <div class="table-responsive">
                        <table class="table table-bordered table-hover table-sm align-middle mb-0" style="background:#fff;font-size:0.70rem;">
                            <thead class="table-light">
                                <tr style="font-size:0.70rem;">
                                    <th class="text-center" style="width:32px;">#</th>
                                    <th class="text-center" style="width:180px;white-space:nowrap;">Etapa</th>
                                    <th class="text-center" style="width:70px;">Distancia<br><small>(Km)</small></th>
                                    <th class="text-center" style="width:90px;">Duración<br><small>(HH:MM)</small></th>
                                    <th class="text-center" style="width:90px;">Espera<br><small>(HH:MM)</small></th>
                                    <th class="text-center" style="width:60px;">Llegada</th>
                                    <th class="text-center" style="width:60px;">Embarque</th>
                                    <th class="text-center" style="width:40px;">Subida</th>
                                    <th class="text-center" style="width:40px;">Bajada</th>
                                    <th class="text-center" style="width:40px;">Int.</th>
                                    <th style="width:32px;"></th>
                                </tr>
                            </thead>
                            <tbody>
                                ${ciudad.etapas.map((etapa, idx) => `
                                <tr>
                                    <td class="text-center">${idx+1}</td>
                                    <td class="text-center fw-normal" style="width:180px;white-space:nowrap;">${etapa.origen}</td>
                                    <td><input type="text" class="form-control form-control-sm text-center stage_distance" style="max-width:60px;padding:2px 4px;" value="${etapa.distancia}" size="4"></td>
                                    <td>
                                        <input type="number" class="form-control form-control-sm d-inline-block text-center t-hr nostyl-sel" style="width:38px;padding:2px 4px;" min="0" max="23" value="${etapa.duracion.split(':')[0]}"> :
                                        <input type="number" class="form-control form-control-sm d-inline-block text-center t-min nostyl-sel" style="width:38px;padding:2px 4px;" min="0" max="59" value="${etapa.duracion.split(':')[1]}">
                                    </td>
                                    <td>
                                        <input type="number" class="form-control form-control-sm d-inline-block text-center t-hr nostyl-sel" style="width:38px;padding:2px 4px;" min="0" max="23" value="${etapa.espera.split(':')[0]}"> :
                                        <input type="number" class="form-control form-control-sm d-inline-block text-center t-min nostyl-sel" style="width:38px;padding:2px 4px;" min="0" max="59" value="${etapa.espera.split(':')[1]}">
                                    </td>
                                    <td class="text-center"><span class="arr_time rec" style="font-size:0.70em;">${etapa.llegada}</span></td>
                                    <td class="text-center"><span class="calc_time rec" style="font-size:0.70em;">${etapa.embarque}</span></td>
                                    <td class="text-center"><input type="radio" name="subida_${cidx}_${idx}" ${etapa.subida?'checked':''}></td>
                                    <td class="text-center"><input type="radio" name="bajada_${cidx}_${idx}" ${etapa.bajada?'checked':''}></td>
                                    <td class="text-center"><input type="checkbox" ${etapa.integracion?'checked':''}></td>
                                    <td class="text-center"><button type="button" class="btn btn-danger btn-xs btn-del-etapa" data-ciudad="${ciudad.nombre}" data-idx="${idx}" title="Eliminar etapa"><i class="fa fa-trash"></i></button></td>
                                </tr>
                                `).join('')}
                            </tbody>
                        </table>
